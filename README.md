...
### 1.1 배경: "월급쟁이 후회의 삼각지대를 넘어서"

**"그때 비트코인 살걸", "그때 집 살걸", "그때 그 주식 살걸."**

우리는 늘 지나간 차트를 보며 후회합니다. 이를 일명 **'월급쟁이 후회의 삼각지대'** 라고 부릅니다. 최근 고물가와 원화 가치 하락이 겹치며, 이제 투자는 단순한 재테크 수단이 아닌 **'생존의 문제'** 가 되었습니다.

하지만 준비 없이 뛰어든 시장은 냉혹합니다. 정보의 비대칭성, 뇌동매매, 그리고 감당하기 힘든 변동성 속에서 개인 투자자는 **"도대체 어디에 투자해야 할지 모르겠다"**는 막막함에 빠집니다. 눈과 뇌가 제 기능을 못 하고 감정적인 결정을 반복하는 이유, 바로**명확한 '판단의 기준'이 없기 때문**입니다.

### 1.2 프로젝트 정의: "직관을 구조로 바꾸다"

본 프로젝트는 막연한 감이나 소문에 의존하던 투자를 멈추고,**데이터(Data)에 기반한 논리적인 투자 기준**을 제시하기 위해 기획되었습니다.

**"영웅호걸의 시간은 시장이 열릴 때가 아니라, 바로 이 '판단 기준을 세울 때' 시작됩니다."**

우리는 다음 3단계의 체계적인 시스템을 통해**2026년 시장을 주도할 기회**를 포착합니다.

1. **Prediction (예측)** : 머신러닝(Prophet+XGBoost)을 활용하여**2026년 유망 섹터를 식별**합니다.
2. **Classification (분류)** : 클러스터링을 통해**투자자의 성향(단기/장기)에 맞는 종목을 분류**합니다.
3. **Verification (검증)** : 기술적 지표와 리스크 분석을 통해**안정성을 검증**합니다.
...
* **대응**: Prophet 모델은 시계열 데이터에서 **'변곡점'** 을 감지합니다. 최근 데이터의 하락 기울기가 급해지면, 향후 예측값($\hat{y}$)을 자동으로 하향 조정하여 해당 섹터를 추천 순위에서 즉시 배제합니다.
