...
**Main Pipeline**: `Multi_Horizon_Industry_Pipeline.ipynb`
**Focus**: Multi-Horizon Sector Prediction + Industry Clustering by Horizon

**Quick Start**:

```bash
conda activate py_study
# 전체 파이프라인 실행 (예측부터 클러스터링까지)
jupyter notebook Multi_Horizon_Industry_Pipeline.ipynb
```

**Key Update**:

- 섹터 통합 로직을 제거하고, **7가지 호라이즌(1d~1y)별로 독립적인** Top-3 섹터를 선정합니다.
- 선정된 섹터 내에서 리스크-수익률 기반의 **인더스트리 클러스터링**을 수행하여 최적의 투자 대상을 식별합니다.

### 1.1 이전 구현과의 차이점 (Major Changes)

| 구분            | 현재 구현                                    |     |
| :-------------- | :------------------------------------------- | :-- |
| **섹터 선정**   | **각 호라이즌별 독립적인** Top-3 섹터 선정   |     |
| **클러스터링**  | **호라이즌별 유망 섹터 내** 산업 군집 분석   |     |
...
- **Log_Volume_W** : 거래량 데이터의 왜도를 완화하기 위해 로그 변환 및 윈저라이징(Capping) 적용.
...
선정된 상위 섹터 내 산업들을 리스크와 수익률 지표를 기반으로 4개 그룹으로 군집화합니다.

- **Cluster 2 (공격형)**: 고수익, 고위험 (성장 중심)
- **Cluster 1 (최적형)**: 최적 리스크-보상 (핵심 포트폴리오 대상)
- **Cluster 0 (안정형)**: 중립적/낮은 변동성 (완충재)
- **Cluster 3 (주의형)**: 가치 함정 (낮은 수익률, 높은 낙폭)
- **Parameters**: `n_clusters=4`, `lookback_days=90` (산업 특성 계산 기간)
...
### 6.2 산업별 군집 분석 (Clustering)

- 선정된 섹터 내 산업들을 `Return_3M`, `Volatility_20d`, `MDD`, `Sharpe_Ratio`를 기준으로 K-Means 알고리즘을 사용하여 분류합니다.

### ~~6.3 옥석 가리기 (Screening)~~

- ~~유망 클러스터(Cluster 1, 2) 내에서 '변동성은 제어되면서 기대 수익이 높은' 산업을 최종 추출합니다.~~

### 6.4 워크플로우 예시

- **1일 호라이즌 (1d) 분석 예시**:
  - **Top-3 섹터**: Energy, Financial Services, Technology 선정.
  - **산업 분석**: 해당 섹터 내 45개 산업을 리스크-수익률로 분석.
  - **결과**: 4개 클러스터로 분류하여 `20260116_1d_*.csv` 파일 5개 생성.
- **1개월 호라이즌 (1m) 분석 예시**:
  - **Top-3 섹터**: Healthcare, Technology, Consumer Defensive 선정.
  - **산업 분석**: 해당 섹터 내 52개 산업 분석.
  - **결과**: 4개 클러스터로 분류하여 `20260116_1m_*.csv` 파일 5개 생성.

각 호라이즌은 독립적으로 분석되므로, 투자 시계(Time Horizon)에 따라 서로 다른 주도 섹터와 유망 산업군을 확인할 수 있습니다.
...
- **CSV 파일 (총 35개)**:
  ...
  - **클러스터별 목록**: `{date}_{horizon}_cluster_{n}.csv` (7개 호라이즌 × 4개 클러스터 = 28개)
- **Tableau 연동**: 생성된 CSV 데이터를 기반으로 섹터 로테이션 히트맵 및 Risk-Reward 산점도 시각화 가능.

## 8. 향후 발전 계획

1. **데이터 업데이트**: 최신 시장 데이터를 반영하여 정기적인 파이프라인 실행 및 결과 갱신.
2. **클러스터 검증**: 도출된 유망 클러스터의 실제 성과를 통한 모델 신뢰도 확보.
3. **시각화 강화**: Tableau를 활용하여 클러스터별 Risk-Reward 산점도 시각화 대시보드 구축.
4. **포트폴리오 자동화**: 클러스터 분석 결과를 기반으로 한 최적 자산 배분 알고리즘 연동.
